...
> **Historical planning doc.** The brand has since shipped a hot-tub identity
> (blue-water mark). The canonical, Figma-ready asset package now lives in the
> top-level **`branding/`** folder (SVG + PNG for everything; see
> `branding/README.md`), generated by `brand/scripts/export.mjs`. Paths below
> that point at `brand/logo/…` are superseded by `branding/…`.
